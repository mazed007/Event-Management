<html>
	<head>
		<title>Tanis Event Management</title>
 ...
 <!--  <a class="navbar-brand" href="#">Navbar</a>-->
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item">
        <a class="nav-link" href="{{ url('/centerowner/confirmbook') }}">Confirm Book</a>
      </li>
      <li class="nav-item dropdown">
        ...
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          <a class="dropdown-item" href="{{ url('/customer/login') }}">Customer</a>
          <a class="dropdown-item" href="{{ url('/centerowner/login') }}">Center Owner</a>
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="{{ url('/eventmanager/login') }}">Event Manager</a>
        </div>
      </li>
      <li class="nav-item">
     ...
     @if (Auth::guest())
                            <a href="{{ url('/centerowner/login') }}">Login</a> &nbsp &nbsp &nbsp &nbsp
                            <a href="{{ url('/centerowner/register') }}">Register</a>
                        @else
                            <li class="dropdown">
                                <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">
                                    {{ Auth::user()->owner_name }} <span class="caret"></span>
                                </a>

                                <ul class="dropdown-menu" role="menu">
                                    <li>
                                        <a href="{{ url('/centerowner/logout') }}"
                                            onclick="event.preventDefault();
                                                     document.getElementById('logout-form').submit();">
                                            Logout
                                        </a>

                                        <form id="logout-form" action="{{ url('/centerowner/logout') }}" method="POST" style="display: none;">
                                            {{ csrf_field() }}
                                        </form>
                                    </li>
                                </ul>
...
@yield('content')
	...
	<footer class="footer-bottom">
		<p class="text-center">&copy; 2019 All rights reserved | Tanis Event Management</p>
